...
def test__to_numpy_array__when_grid_type_1__expect_correct_array():
    ...
    y, x = (3, 2)
    grid = Grid(
        minimum_north_position=48.143304983,
        minimum_east_position=15.141245418,
...
def test__from_numpy_array_to_type_2__when_multi_pdv__expect_correct_bytes():
    ...
    grid_bin = from_numpy_array_to_type_2(arr, grid, number_pdv=pdv)
    assert grid_bin == expected_bin


def test__to_numpy_array__when_grid_type_2__expect_correct_array():
    y, x, pdv = (2, 2, 2)
    grid_bin = (
            b'\x00\x00\x00\x00' + b'\x00\x00\x00\x00'
            + b'\x01\x00\x00\x00' + b'\xFF\xFF\xFF\xFF'
    ...
    arr_actual = to_numpy_array(grid_bin, grid, number_pdv=pdv)
    assert np.array_equal(arr_expected, arr_actual)
